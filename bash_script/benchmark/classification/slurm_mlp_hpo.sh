...
conda activate ag
cd /path/to/output/mlp_handcrafted_feature_results
  ...
  for lr in "${lrs[@]}"; do
    for do in "${dropouts[@]}"; do
      if [ "$idx" -eq "$SLURM_ARRAY_TASK_ID" ]; then
        ...
        srun python /path/to/benchmark/classification/mlp_pl2_wloss_standardization.py \
          --batch_size  "$bs" \
          --lr          "$lr" \
          --dropout     "$do" \
          ...
          --seed        "$seed" \
          --input_embs  "$INPUTS" \
          --out_dir     "/path/to/output/classification/mlp" \
          --epochs      50 \
          --scenario    concat \
          --hand_crafted 1
        exit 0
